...
def _apply_omission_paths(result: Any, generator: Any) -> None:
    ...
    for klass, path, kind, withholds_default in work:
        try:
            ...
            if apply_to_operations(result.get("paths", {}), fields, path, kind, withholds_default):
                continue
            name = component_names.get(klass)
            # A partial schema -- the apidocs tests generate one per endpoint --
            # need not contain every declaring serializer. The path was already
            # resolved against the code, so there is nothing left to check.
            if name is not None and name in schemas:
                apply_path(schemas, name, path, kind, declarations, parents)
        except OmissionError as exc:
